...
public class UsersController {
    ...
    public R getUsers(@RequestBody PageIn pageIn) {
        ...
        PageOut pageOut = new PageOut();pageOut.setList(outs);
    ...
    public R getReader(@RequestBody PageIn pageIn) {
        if (pageIn == null) {
            return R.fail(CodeEnum.PARAM_ERROR);
        }
        // 封装分页出参对象
        PageInfo<Users> userList = userService.getReaderList(pageIn);
        List<UserOut> outs = new ArrayList<>();
        for (Users users : userList.getList()) {
            UserOut out = new UserOut();
            BeanUtils.copyProperties(users,out);
    ...
    public R addMsg(@RequestBody Msg msg) {
        return R.success(CodeEnum.SUCCESS,msgService.addMsg(msg));
    }

    @ApiOperation("删除留言")
    @GetMapping("/deleteMsg")
    public R deleteMsg(Integer id) {
    ...
    @ApiOperation("留言列表")
    @PostMapping("/msgList")
    public R getMsgList(@RequestBody PageInNoKey PageInNoKey) {
        if (PageInNoKey == null) {
            return R.fail(CodeEnum.PARAM_ERROR);
        }

            return R.success(CodeEnum.SUCCESS,msgService.getMsgList(PageInNoKey));
    }
...
//    }
    ...
    @ApiOperation("判断重名")
    @PostMapping("/isExit")
    public List<Users> isExit(String username) {
        return userService.isExit(username);
    }
    ...
    public R getCurrUser() {
        ...
        if (principal!=null) {
            ...
            if (StrUtil.isNotBlank(username)) {
                ...
                }else if (identity == Constants.NL) {
                    ident = Constants.NULL_STR;
                }
                out.setIdent(ident);
                return R.success(CodeEnum.SUCCESS,out);
            }
